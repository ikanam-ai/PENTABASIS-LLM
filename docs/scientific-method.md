# Научный метод PENTABASIS-LLM v0.1
...
**RQ5. Адаптивность.** Добавляют ли адаптивные сценарии различающую способность, сохраняя содержательную связь с PENTABASIS-LLM?
...
`40` канонических ситуаций создаются из кодировочной книги PENTABASIS-LLM и фиксируются до анализа ответов оцениваемой панели.
...
flowchart LR
    ...
    G[Экспертный gold] --> L
    L --> P[Concept pool PENTABASIS-LLM]
...
Эксперты калибруют измеритель. PENTABASIS-LLM v0.1 не собирает распределение ценностей населения и не использует ответы людей как нормативный эталон.
